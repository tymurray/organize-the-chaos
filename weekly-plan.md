# Weekly Plan - January 26-31, 2026 (Rest of January Sprint)

## 🗓️ REST OF JANUARY SPRINT (Jan 26-31)
**Theme:** Big Sky Competition & Q1 Planning Push

### Sprint Goals (by Jan 31):
💼 **Work:**
- [ ] Q1 2026 QTI plan finalized and shared with leads (status unclear - check today)
- [✓] Stats on SoFi <-> Encompass sync delivered to Aravind (DONE Wed Jan 28)
- [ ] Engineer feedback completed (DUE TODAY Thu Jan 29)

🏠 **Personal:**
- [ ] Big Sky competition (Jan 30-31, weather day Feb 1) executed smoothly
- [✓] Whitefish Airbnb booked (Feb 5-8)
- [✓] New car for Lali (Subaru traded in)
- [ ] Pinkerton: Start prep work if time allows

**Sprint review:** Friday, Jan 31 at 4 PM

---

## 🗓️ FEBRUARY 2026 SPRINT (Feb 1-28)
**Theme:** Competition Season & Steady Pinkerton Progress

### Sprint Goals (by Feb 28):
💼 **Work:**
- [ ] Q4 2025 final QTI wins report completed
- [ ] Q1 2026 mid-quarter check-in completed

🏠 **Personal:**
- [ ] Whitefish competition (Feb 5-8) executed smoothly
- [ ] Presidents Weekend family ski trip (Feb 13-18) executed smoothly
- [ ] Pinkerton: Quarter round + nail holes + sanding complete
- [ ] Pinkerton: Wall prep started (realistic given limited weeknight availability)
- [ ] Backcountry gear research started (skis, touring boots, skins)

**Sprint review:** Friday, Feb 28 at 4 PM

**Note:** Limited Pinkerton work availability in February due to:
- Big Sky competition (Jan 30-31, weather day Feb 1)
- Whitefish competition (Feb 5-8)
- Presidents Weekend ski trip (Feb 13-18)
- Estimated ~8-10 weeknights available for painting/prep

---

## THIS WEEK'S BALANCE

💼 **Work deliverables:**
- [ ] Complete QTI for Q1 2026 and share with leads - status unclear, check today
- [✓] Stats on SoFi <-> Encompass sync to Aravind - DONE Wed Jan 28 (SPRINT GOAL)
- [ ] Complete feedback for engineers - DUE TODAY Thu Jan 29 (SPRINT GOAL - CAN'T MISS!)
- [ ] **Palantir POC work - HIGH PRIORITY** (start if time allows today)

🏠 **Personal must-dos:**
- [✓] Book Airbnb for Whitefish competition (Feb 5-8) - DONE
- [✓] New car for Lali (Subaru traded in) - DONE
- [ ] Prep for Big Sky competition (Jan 30-31, weather day Feb 1)
- [ ] Pinkerton work: Start quarter round or nail hole filling if evening available

**Energy level:** ⚡⚡⚡⚪⚪ (3/5 - Managing multiple priorities, Palantir urgency increased)

---

## 💼 WORK PROJECTS (Max 2 active)

### QTI Planning & Reporting
**Progress:** ████░░░░░░ 40% (2 of 5 milestones) | **Sprint target:** 60%
**Started:** Jan 5, 2026 | **Last worked:** Jan 28
**Next milestone:** Q1 2026 plan finalized (status unclear)
**This week:** Check Q1 plan status + complete if needed

**Milestones:**
- [✓] Q4 2025 initial review complete
- [→] **Q1 2026 plan finalized (status unclear - check today)**
- [ ] Q4 2025 final wins report (Feb)
- [ ] Q1 2026 mid-quarter check-in (Feb)
- [ ] Q2 2026 planning kickoff (beginning of March)
- [ ] Q1 2026 final report (March 31)

**Notes:**
- Stats delivery completed Wed Jan 28 ✓
- Need to verify Q1 plan status and finalize if pending

---

### Palantir Validation & Orchestration Layer
**Progress:** ░░░░░░░░░░ 0% (0 of 4 milestones) | **Sprint target:** 25%
**Started:** Jan 27, 2026 | **Last worked:** Not started yet
**Next milestone:** Requirements gathering/discovery → POC
**This week:** Begin discovery work today if time allows (leaving for Big Sky tonight)
**PRIORITY SHIFT:** Palantir is ready to push hard on POC - HIGH PRIORITY

**Milestones:**
- [→] **Requirements gathering / discovery - CURRENT FOCUS**
- [→] **Proof of concept - PALANTIR READY TO PUSH HARD**
- [ ] Architecture design
- [ ] Share with stakeholders for direction validation

**Notes:**
- Build a Palantir-based validation and orchestration layer between POS origination systems and Encompass (LOS)
- **NEW: Palantir is ready to accelerate POC work - this is now high priority**
- Next 2 weeks: Provide updates to Aravind and Todd
- Final milestone: Present to stakeholders to validate chosen direction

---

### Background Projects (Deprioritized)

**Observability Improvements (Funnel + Temporal)** - 80% complete, ready for codathon when scheduled

---

## 📚 PROFESSIONAL DEVELOPMENT

### Kotlin Coroutines Mastery
**Goal:** Become an expert in Kotlin coroutines
**Progress:** ░░░░░░░░░░ 0% (Getting started)
**Started:** Jan 27, 2026

**Learning Path:**
- [ ] Understand coroutine basics (launch, async, suspend functions)
- [ ] Master structured concurrency and scopes
- [ ] Learn coroutine contexts and dispatchers
- [ ] Understand channels and flows
- [ ] Practice error handling and cancellation
- [ ] Apply to real-world scenarios in codebase

**Resources to explore:**
- Official Kotlin coroutines documentation
- "Kotlin Coroutines: Deep Dive" book
- Internal codebase examples
- Build sample projects

**Weekly commitment:** Dedicate time during slower work periods or "If Time Permits" slots

---

## 💪 HEALTH & FITNESS

### Weight Loss Goal
**Target:** 200 lbs by end of April 2026
**Current:** 227 lbs (as of Jan 27)
**Starting:** 231 lbs (early January)
**Progress:** ███░░░░░░░ 13% (4 of 31 lbs lost)
**Remaining:** 27 lbs to lose
**Timeline:** ~13 weeks remaining (~2 lbs/week target)

**Milestones:**
- [✓] Lost first 4 lbs (231 → 227)
- [ ] Reach 220 lbs (7 more lbs)
- [ ] Reach 210 lbs (17 more lbs)
- [ ] Reach 200 lbs - TARGET (27 more lbs)

**Key Strategies:**
- Regular CrossFit workouts (3-4x per week goal)
- Consistent calorie tracking/awareness
- Sustainable habits over quick fixes

**Weekly Check-ins:** Track weight trend, not daily fluctuations

---

## 🏠 PERSONAL PROJECTS (Max 2 active)

### Javin's Ski Season Support
**Progress:** ████░░░░░░ 40% (2 of 7 milestones) | **Sprint target:** 50%
**Started:** Jan 1, 2026 | **Last worked:** Jan 25
**Next milestone:** Big Sky competition execution
**This week:** Prep for Big Sky + book Whitefish Airbnb

**Milestones:**
- [✓] Jackson Hole competition (Jan 7-8) - Complete
  - Travel, lodging, gear all handled successfully
- [→] **Big Sky competition (Jan 30-31, weather day Feb 1) - CURRENT FOCUS (TOMORROW!)**
  - [✓] Registration complete
  - [✓] Lodging set (staying at condo)
  - [✓] Boot straps ordered for shin bang
  - [✓] Ski tune scheduled (First Chair Services - pickup TODAY!)
  - [→] Shin routine established (ice + ibuprofen + strengthening - do before leaving)
  - [ ] Gear packed (TODAY before leaving)
  - [→] Jan 30: Ski ninja volunteer (check-in 8 AM at BSSEF team room)
- [ ] Whitefish competition (Feb 5-8)
  - [✓] Book Airbnb - DONE
  - [ ] Competition prep
- [ ] Presidents Weekend family ski trip at Whitefish (Feb 13-18)
  - [ ] Lodging
  - [ ] Plan activities
- [ ] Competition skis purchase (end of season timing - March/April)
- [ ] Backcountry gear setup (end Feb/start March)
  - [ ] Research and purchase skis
  - [ ] Touring boots
  - [ ] Re-use shift bindings from K2 Mind Benders
  - [ ] Skins for new backcountry skis
- [ ] First backcountry ski trips together (late Feb/early March)

**Notes:**
- Ski season ends April 26, 2026
- More competitions may be added after Whitefish

---

### Pinkerton House Renovation
**Progress:** ███░░░░░░░ 27% (2.5 of 9 milestones) | **Sprint target:** 30%
**Started:** Jan 1, 2026 | **Last worked:** Jan 26 (Monday evening)
**Next milestone:** Finish last quarter round piece, then fill remaining nail holes
**This week:** Limited availability (Big Sky Thu-Sat), but made good progress Monday

**Milestones:**
- [✓] Trim work complete
- [→] **Quarter round installed under cabinets - 95% DONE (1 piece left in kitchen)**
- [→] **Fill nail holes (all trim and quarter round) - IN PROGRESS (kitchen done)**
- [ ] Sand trim and quarter round
- [ ] Wall prep for painting (all rooms except Javin's)
- [ ] Paint all rooms (except Javin's)
- [ ] Paint kitchen cabinets
- [ ] Final odds and ends complete
- [ ] **Listed (mid-April target)**
- [ ] Sold

**Timeline Notes:**
- Work weeknights when available during February (limited by ski trips)
- February availability: ~8-10 weeknights (lose Jan 30-Feb 1, Feb 5-8, Feb 13-18)
- March: More availability as competition schedule lightens
- Early April: Final push before mid-April listing
- Ski season ends April 26, full focus available after

**Critical Path:**
Quarter round → Fill holes → Sand → Wall prep → Paint rooms → Paint cabinets → Final touches → List

**Work happens when we can during weeknights - realistic progress given ski season priorities**

---

## COMPLETED: Monday, January 26
✓ Whitefish Airbnb booked | ✓ Interview + scorecard | ✓ Pinkerton: Nail holes + quarter round (1 piece left) | ✓ New car for Lali (evening - Subaru traded in)
⏳ Stats for Aravind (rolled to Tue)

---

## COMPLETED: Tuesday, January 27
✓ 11:15 AM Staff Engineer interview | ✓ Scorecard submitted | ✓ Booster strap ordered for Javin | ✓ 12:00 PM CrossFit workout
⏳ Stats for Aravind (rolled to Wed) | ⏳ QTI Q1 2026 plan (rolled to Wed)

---

## COMPLETED: Wednesday, January 28
✓ Called First Chair Services - ski tune scheduled | ✓ Stats for Aravind sent (SoFi <-> Encompass sync data) - CRITICAL DEADLINE MET! | ✓ Helped Lali remove cargo box from Subaru (4:30-5:00 PM) | ✓ 5:30 PM CrossFit workout | ✓ 10:00 AM 1:1 with Todd | ✓ 10:15 AM Daily Standup | ✓ 11:30 AM HL Eng All Hands | ✓ 12:00 PM Monthly Borrow Design Review | ✓ 1:45 PM Manager interview (Senior Manager Software Engineer)
⏳ QTI Q1 2026 plan (status unclear - may still be pending)

---

## TODAY: Thursday, January 29

### ⏰ MORNING REVIEW (5 min - Do this FIRST)
- [ ] Work calendar events reviewed (check below)
- [ ] Personal commitments added to calendar section
- [ ] **Workout time chosen** - TBD based on schedule
- [ ] Today's priorities are realistic given available time
- [ ] Clear mental picture of the day's flow
- [ ] Top priority identified: Engineer feedback drafts (DUE TODAY - CAN'T MISS)

**🏋️ Today's Workout Decision:**
- [ ] **TBD** - Choose based on schedule and packing needs

---

### 📅 TODAY'S CALENDAR

**💼 Work Commitments:**
- 10:15-11:15 AM - HLID Backlog Review (optional/skip)
- 11:30 AM-12:00 PM - HL: Weekly Incident + On-Call Review (optional/skip)
- 12:00-1:00 PM - Data Privacy Week Trivia (optional/skip)
- **2:30-2:55 PM - HEX Platform Observability (REQUIRED - only meeting to attend!)**
- 3:30-4:15 PM - SoFi - Brighthire Training for EPD (optional/skip)

**🏠 Personal Commitments:**
- **8:15 AM:** Nidia's lash appointment (may affect morning routine)
- **Afternoon:** Pick up Javin and Ty's skis from First Chair Services (MUST DO before leaving!)
- **Late afternoon/Evening:** Drive to Four Corners - stay the night before Big Sky competition
- **Before leaving:** Javin shin routine (ice + ibuprofen + strengthening exercises)
- **Before leaving:** Finish packing gear for Big Sky

**⏰ Available Work Blocks:**
- **8:30 AM-2:30 PM (~6 hours)** - Long uninterrupted focus block (only interrupted by 2:30 meeting!)
- **2:55-4:30 PM (1h 35min)** - Afternoon block after HEX meeting
- **After 4:30 PM** - Need to pick up skis and leave for Four Corners
- **Total: ~7h 35min deep work time available (excellent for engineer feedback!)**

---

### 🎯 TODAY'S PRIORITIES

🔥 **CAN'T MISS (Just 1)**
- [ ] Engineer feedback: Complete first drafts (DUE TODAY - SPRINT GOAL!)

💼 **Work Focus (Top 3)**
- [ ] Palantir POC work: Start discovery/requirements gathering (HIGH PRIORITY - if time allows)
- [ ] QTI Q1 2026: Check status and finalize if not done yesterday
- [ ] Attend key meetings: HLID Backlog Review, Incident Review

🏠 **Personal MUST DO (Leaving for Big Sky tonight!)**
- [ ] Pick up Javin and Ty's skis from First Chair Services (CRITICAL - before leaving!)
- [ ] Finish packing gear for Big Sky competition
- [ ] Javin shin routine before leaving (ice + ibuprofen + strengthening exercises)
- [ ] Drive to Four Corners for overnight stay

📋 **If Time Permits**
- [ ] Review Big Sky schedule with Javin
- [ ] Attend Data Privacy Week Trivia (12:00-1:00 PM)

---

### ✅ COMPLETED SO FAR TODAY
- (Will update as day progresses)

---

### 🌙 END OF DAY REVIEW (5 min - Do this LAST)

**Completion Check:**
- [ ] CAN'T MISS item status: [Done / In Progress / Blocked - explain why]
- [ ] Work Focus items: [List what got done]
- [ ] Personal MUST DO items: [List what got done]

**Tomorrow Planning:**
- [ ] Big Sky competition starts tomorrow! (Fri Jan 30)
- [ ] Ski ninja volunteer duty - check-in at 8 AM at BSSEF team room
- [ ] Any last-minute items for Big Sky noted below

**Brain Dump (write anything causing anxiety):**
-
-
-

**Tomorrow Preview:**
- Friday, Jan 30: Big Sky competition begins
- 8:00 AM check-in at BSSEF team room
- All day ski ninja volunteer duty

---

## THIS WEEK - Key Deadlines

### Monday, Jan 26 - COMPLETE
✓ Whitefish Airbnb booked | ✓ Interview + scorecard | ✓ Pinkerton work | ✓ New car for Lali (evening)
⏳ Stats rolled to Tue

### Tuesday, Jan 27 - COMPLETE
✓ 11:15 AM Staff Engineer interview | ✓ Scorecard submitted | ✓ Booster strap ordered | ✓ 12:00 PM CrossFit workout
⏳ Stats rolled to Wed | ⏳ QTI rolled to Wed

### Wednesday, Jan 28 - COMPLETE
- ✓ Called First Chair Services (ski tune scheduled)
- ✓ Stats for Aravind delivered - CRITICAL DEADLINE MET!
- ✓ 1:1 with Todd | ✓ Daily Standup | ✓ HL Eng All Hands | ✓ Monthly Borrow Design Review
- ✓ Manager interview (Senior Manager Software Engineer)
- ✓ Helped Lali with Subaru cargo box (4:30-5:00 PM)
- ✓ 5:30 PM CrossFit workout
- ⏳ QTI Q1 2026 plan (status unclear - may need follow-up)

### Thursday, Jan 29 (TODAY) 🔥🔥🔥
- **8:15 AM:** Nidia's lash appointment (may affect morning routine)
- **2:30 PM:** HEX Platform Observability (ONLY required meeting - 25 min)
- **🔥 CRITICAL DEADLINE TODAY:** Engineer feedback drafts (DUE TODAY - SPRINT GOAL!)
- **🔥 CRITICAL BEFORE LEAVING:** Pick up skis from First Chair Services (MUST DO!)
- **Excellent work time:** ~7.5 hours available for deep work before leaving
- **Evening:** Drive to Four Corners for overnight stay
- **Before leaving:** Pack gear + Javin shin routine

### Friday, Jan 30
- **8:00 AM:** Check-in at BSSEF team room for ski ninja volunteer duty
- **All day:** Big Sky competition - ski ninja volunteer

### Saturday, Jan 31
- **All day:** Big Sky competition continues

### Sunday, Feb 1
- **Weather day:** Big Sky competition (if needed)
- **Evening:** Sprint review - assess week's progress (if home)

---

## NEXT WEEK PREVIEW (Feb 2-8)

### Key events:
- **Wed-Sat, Feb 5-8:** Whitefish competition (need Airbnb booked!)
- **Work:** Focus on Q4 2025 final wins report
- **Pinkerton:** Limited availability due to Whitefish trip

---

## UPCOMING (Beyond Next Week)

### February
- **Feb 13-18:** Presidents Weekend family ski trip at Whitefish (Fri-Tue)
- **Mid-Feb:** Q1 2026 mid-quarter check-in
- **Late Feb:** Start backcountry gear research and purchases

### March
- **Early March:** Q2 2026 planning kickoff
- **March:** Increased Pinkerton house work (fewer ski trips)
- **March 31:** Q1 2026 final report due

### April
- **Mid-April:** Pinkerton house listing target
- **April 26:** Ski season ends - full focus available for house completion

---

## REMINDERS & NOTES

### 🚨 Don't Forget
- **New car for Lali** - ✓ DONE (Subaru traded in Mon evening)
- **Whitefish Airbnb** - ✓ BOOKED for Feb 5-8
- **Big Sky Jan 30** - Check-in 8 AM at BSSEF team room (ski ninja volunteer)

### ⚡ Energy Management
- This week is HEAVY: Engineer feedback DUE TODAY + Leaving for Big Sky tonight
- Protect TODAY for engineer feedback - TOP PRIORITY
- Limited time today - need to leave for Four Corners by late afternoon/evening
- Accept limited work time this week due to Big Sky competition

### 🎯 Focus Areas
- **Work:** Engineer feedback is TOP priority TODAY (CAN'T MISS!)
- **Work:** Palantir POC work if time allows (HIGH PRIORITY but realistically next week)
- **Personal:** Big Sky logistics (ski pickup TODAY, pack gear, shin routine before leaving)
- **House:** No Pinkerton work this week due to Big Sky

### 💡 System Notes
- Using Option 3: Monthly Sprint Layer
- 3 active projects (QTI, Javin's Season, Pinkerton)
- If overhead becomes too much, can simplify to Option 2
- Sprint reviews: End of each month to assess progress and adjust

### 🏋️ Workout Planning System
- **Goal:** Workout every workday if possible, realistically 3-4 days/week in busy February
- **Time options:** Noon (light meeting days) | 4:30 PM (end work early) | 5:30 PM (no Pinkerton nights)
- **Morning Review:** Choose workout time based on day's calendar and Pinkerton plans
- **Default on Pinkerton nights:** Noon or 4:30 PM (avoid 5:30 PM to protect house work time)
- **Flexibility:** Skip workout if day is overloaded or critical deadline

---

## COMPLETED THIS WEEK
- [ ] (Will update as week progresses)
